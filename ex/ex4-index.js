	...
	async handle(handlerInput) {
		...
		return await ms.getInSkillProducts(locale).then(function (res) {
			...
			if (handlerInput.requestEnvelope.request.status.code === '200') {
				...
				let speechText = products[0].summary;
				const repromptText = '続けますか？';
				...
				switch (handlerInput.requestEnvelope.request.payload.purchaseResult) {
					case 'ACCEPTED':
						// 購入した
						speechText = products[0].summary + repromptText;
						break;
					case 'DECLINED':
						// 購入しなかった
					...
					default:
						// その他のERROR
						speechText = `うまく行かなかったようですが、${products[0].name} にご興味をいただき、ありがとうございました。${repromptText} `;
						break;
				}
				// ユーザーに応答を返す
				return handlerInput.responseBuilder
